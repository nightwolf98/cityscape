...
public class ItemHumvee extends Item
{
    public ItemHumvee(int par1)
    {
        super(par1);
        this.maxStackSize = 1;
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
                {
                    ...
                    EntityHumvee entityhumvee = new EntityHumvee(par2World, (double)((float)i + 0.5F), (double)((float)j + 1.0F), (double)((float)k + 0.5F));
                    entityhumvee.rotationYaw = (float)(((MathHelper.floor_double((double)(par3EntityPlayer.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

                    if (!par2World.getCollidingBoundingBoxes(entityhumvee, entityhumvee.boundingBox.expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return par1ItemStack;
                    }
                    ...
                    {
                        par2World.spawnEntityInWorld(entityhumvee);
                    }
