...
import minecraft.phoenix.cityscape.core.entity.EntityBullet;
...
public class ItemSpaceRifle extends Item
{
	...
	private int reloadmax; // The value that the reloadtick variable needs to be
							// in order for the gun to reload
	private int clipid = 204; // The item id of our clip
	private int ammo; // The amount of ammo avaliable per clip (used in setting
						// durability)
	private int firetick; // The current value of our shooting timer
	private int firemax; // The value that the firetick variable needs to be in
							// order for the gun to fire
	// Fire delay is completely ignored when firemax is set to 0
	private String firesound; // The String that notch uses in his code to
								// figure out what sound to play when we shoot
								// our gun
	private String reloadsound; // Firesound but with reloading
	
	/*
	 * /* The parameters for our constructor are: /* int i (the id), int damage
	 * (damage in half-hearts), int ammo (how much /* ammo per clip) /* int
	 * clipid (the item id of the clip this gun uses), int firedelay (the /*
	 * value of firemax) /* String firesound (the value of firesound), String
	 * reloadsound (the value /* of reloadsound)
	 */
	public ItemSpaceRifle(int i, int damage, int ammo, int clipid, int firedelay, String firesound, String reloadsound)
	{
		
		super(i); // calls the Item.java constructor and passes in the item id
					// for a parameter
		this.damage = damage; // sets the damage value
		firemax = firedelay; // sets the firemax value
		firetick = firemax; // sets the firetick value equal to firemax (so you
							// don't need to wait for the delay on the first
							// shot)
		...
		setMaxDamage(ammo + 1); // sets the durability of our gun to the ammo
								// count + 1
		this.setCreativeTab(CreativeTabs.tabCombat);
		this.setUnlocalizedName("spaceRifle");
	}
	...
	public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer)
	{
		...
		if (!par2World.isRemote && par1ItemStack.getItemDamage() < ammo)
		{
			...
			// if the firemax isn't 0, and the fireing counter is done, then
			// this is true
			...
			else
			{
				...
				// if the fire delay counter isn't done, then this is called,
				// which increases the fire delay counter
				++firetick;
			}
			
			// if firemax is 0 none of the above was called, so call this
			// instead
			if (firemax == 0)
			{
			...
			// If the player is out of ammo in the current clip, and the player
			// has the correct clip, run this code
		}
	...
	public void onPlayerStoppedUsing(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer, int i)
	{
		...
		// this resets the firetick. I originally planned on using this method
		// for non-automatic weapons
		// but the method didn't seem to end up being called at all... if
		// someone could clarify why this is
